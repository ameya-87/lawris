...
export async function saveTokens(userId: string, tokens: GoogleTokens): Promise<void> {
  ...
  const { error } = await sb.from("calendar_tokens").upsert(
    {
      user_id: userId,
      provider: "google",
  ...
  if (error) {
    console.error("[oauth] saveTokens failed", error);
...
export async function getConnectionStatus(userId: string): Promise<{
  ...
  try {
    ...
    const { data, error } = await sb
      ...
      .maybeSingle();
    if (error) {
      // Table missing / RLS / transient — log and degrade to "disconnected" rather than throw.
      console.warn("[oauth] getConnectionStatus warning:", error.message);
      return { connected: false, connected_at: null, scope: null };
    }
    return {
      connected: !!data,
    ...
    console.warn("[oauth] getConnectionStatus fatal:", e);
    return { connected: false, connected_at: null, scope: null };
  }
}
